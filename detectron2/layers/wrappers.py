...
class ConvP3d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, **kwargs):
        ...
        default_args = {"stride": 1, "padding": 0, "dilation": 1}
        self.stride = _triple(kwargs.get("stride", default_args["stride"]))
        self.padding = _triple(kwargs.get("padding", default_args["padding"]))
        self.dilation = _triple(kwargs.get("dilation", default_args["dilation"]))

        kernel_size_2d = (1,) + self.kernel_size[1:]
        kernel_size_z = (self.kernel_size[0],) + tuple(repeat(1, len(self.kernel_size[1:])))

        kwargs_2d = kwargs.copy()
        kwargs_z = kwargs.copy()
        for k, v in [("stride", self.stride), ("padding", self.padding), ("dilation", self.dilation)] :
            kwargs_2d[k] = (default_args[k],) + v[1:]
            kwargs_z[k] = (v[0],) + tuple(repeat(default_args[k], len(v[1:])))

        self.conv2d = torch.nn.Conv3d(in_channels, out_channels, kernel_size_2d, **kwargs_2d)
        self.convZ = torch.nn.Conv3d(out_channels, out_channels, kernel_size_z, **kwargs_z)
